...
//	step = 25;
	...
	switch (event->data.key_pressed) {
		case KEY_1:
			for (i = base + step * 0; i < base + step * 1; ++i) {
				hPortWrite(i, 1);
				PFprintf("Signal %d to 1.\n", i);
				suSleep(50, NULL);
			}
			break;
		case KEY_2:
			for (i = base + step * 1; i < base + step * 2; ++i) {
				hPortWrite(i, 1);
				PFprintf("Signal %d to 1.\n", i);
				suSleep(50, NULL);
			}
			break;
		case KEY_3:
			for (i = base + step * 2; i < base + step * 3; ++i) {
				hPortWrite(i, 1);
				PFprintf("Signal %d to 1.\n", i);
				suSleep(50, NULL);
			}
			break;
		case KEY_4:
			for (i = base + step * 3; i < base + step * 4; ++i) {
				hPortWrite(i, 1);
				PFprintf("Signal %d to 1.\n", i);
				suSleep(50, NULL);
			}
			break;
		case KEY_5:
			for (i = base + step * 4; i < base + step * 5; ++i) {
				hPortWrite(i, 1);
				PFprintf("Signal %d to 1.\n", i);
				suSleep(50, NULL);
			}
			break;
		case KEY_6:
			for (i = base + step * 5; i < base + step * 6; ++i) {
				hPortWrite(i, 1);
				PFprintf("Signal %d to 1.\n", i);
				suSleep(50, NULL);
			}
			break;
		case KEY_7:
			for (i = base + step * 6; i < base + step * 7; ++i) {
				hPortWrite(i, 1);
				PFprintf("Signal %d to 1.\n", i);
				suSleep(50, NULL);
			}
			break;
		case KEY_8:
			for (i = base + step * 7; i < base + step * 8; ++i) {
				hPortWrite(i, 1);
				PFprintf("Signal %d to 1.\n", i);
				suSleep(50, NULL);
			}
			break;
		case KEY_9:
			for (i = base + step * 8; i < base + step * 9; ++i) {
				hPortWrite(i, 1);
				PFprintf("Signal %d to 1.\n", i);
				suSleep(50, NULL);
			}
