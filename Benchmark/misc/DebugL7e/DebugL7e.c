...
//	WCHAR *text;
	HAPI_CLOCK_REG_T *hapi_clock_reg = (void *) 0x24845000;
#if defined(FTR_L7E)
	UINT8 *hapi_clock_rate_mcu = (void *) 0x3FEFFF0;
#else
	UINT8 *hapi_clock_rate_mcu = (void *) 0x3FC3600;
#endif
	WCHAR num[16];
	WCHAR string_to_out[64];
	app_instance = (APP_INSTANCE_T *) app;
	...
	switch (state) {
		case BENCHMARK_MAIN_SCREEN:
			...
			u_strcpy(string_to_out, num);
			u_strcpy(string_to_out + u_strlen(string_to_out), L" ");

			u_ltou(hapi_clock_reg->ref_dpll_op, num);
			u_strcpy(string_to_out + u_strlen(string_to_out), num);
			u_strcpy(string_to_out + u_strlen(string_to_out), L" ");

			u_ltou(*hapi_clock_rate_mcu, num);
			u_strcpy(string_to_out + u_strlen(string_to_out), num);
			u_strcpy(string_to_out + u_strlen(string_to_out), L" ");

			point.x = 10;
			point.y = 50;
			UIS_CanvasDrawColorText(string_to_out, 0, u_strlen(string_to_out), point, 0, app->dialog);
